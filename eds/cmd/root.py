...
def _resolve_version() -> str:
    """PARITY: main.go — the build-baked Version wins; $GIT_SHA applies ONLY when the baked value is absent/"dev".

    Go: the ldflags-injected `version` (default "dev"); `if version == "dev" { version = $GIT_SHA }`. Here the
    baked value lives in eds._version (written by packaging/build.py, gitignored)."""
    try:
        from eds._version import VERSION as baked  # generated by packaging/build.py (gitignored)
    except ImportError:
        baked = "dev"
    if baked != "dev":
        return baked
