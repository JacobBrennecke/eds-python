...
class NDJSONDecoder:
    ...
    def open(cls, fn: str) -> NDJSONDecoder:
        try:
            if os.path.splitext(fn)[1] == ".gz":
                fp: Any = gzip.open(fn, mode="rt", encoding="utf-8", newline="")
            else:
                fp = open(fn, encoding="utf-8", newline="")
        except OSError as e:
            raise OSError(f"error opening: {fn}. {e}") from e
        return cls(fp, owns=True)
    ...
    def decode_raw(self) -> str:
        ...
        self._peeked = None
        self._has_peeked = False
        json.loads(value)  # validate syntax (Go RawMessage validates but keeps the raw bytes)
        self._count += 1
        return value
